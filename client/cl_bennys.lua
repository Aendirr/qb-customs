...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'mechanic' then
            ...
            QBCore.Functions.Notify('Bazı satın alımlar şu anda kullanılamıyor. Lütfen bir tamirci bulun.')
        end
        ...
        if not canEnter then
            QBCore.Functions.Notify('Burada hiçbir şey yapamazsın!')
            ExitBennys()
            return
        end
        ...
        local welcomeLabel = (locationData and locationData.settings.welcomeLabel) or "Mekanik Menüsü"
...
RegisterNetEvent("qb-customs:client:purchaseFailed", function()
    ...
    attemptingPurchase = false
    QBCore.Functions.Notify("Yeterli para yok.", "error")
end)
